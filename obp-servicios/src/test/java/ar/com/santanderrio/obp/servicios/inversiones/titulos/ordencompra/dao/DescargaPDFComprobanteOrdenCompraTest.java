package ar.com.santanderrio.obp.servicios.inversiones.titulos.ordencompra.dao;
...
public class DescargaPDFComprobanteOrdenCompraTest {
	...
	@Mock
	private ComprobanteOrdenCompra comprobanteOrdenCompra;
	...
	public void testReporteComprobantePDFTransferencia() throws IllegalAccessException, DAOException, IOException {
		...
		Mockito.when(comprobanteOrdenCompra.obtenerParametrosPDF()).thenReturn(obtenerParametrosPDF());
		Mockito.when(comprobanteOrdenCompra.obtenerJasper()).thenReturn(appContext
				.getResource("classpath:/report/comprobantes/inversiones/orden-compra.jasper").getFile().getPath());

		Mockito.when(comprobanteOrdenCompra.getTituloComprobante()).thenReturn("Comprobante de orden compra titulos");

		Reporte reporte = reporteComprobantePDFDAO.obtenerReporteComprobantePDF(comprobanteOrdenCompra);
//		FileUtils.writeByteArrayToFile(new File("C:/tools/comprobanteOrdenCompraTest.pdf"), reporte.getBytes());
	...
	private HashMap<String, Object> obtenerParametrosPDF() {
		...
		parametros.put("CUENTA_DEBITO", "250-354914/0");
		parametros.put("TIPO_CUENTA_DEBITO", "Cuenta unica");
