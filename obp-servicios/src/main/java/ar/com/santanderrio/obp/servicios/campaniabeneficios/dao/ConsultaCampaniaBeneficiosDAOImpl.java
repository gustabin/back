...
public class ConsultaCampaniaBeneficiosDAOImpl extends BaseDatoDAOImpl implements ConsultaCampaniaBeneficiosDAO {
    /** The Constant LOGGER. */
    private static final Logger LOGGER = LoggerFactory.getLogger(ConsultaCampaniaBeneficiosDAOImpl.class);
    ...
    private static final String CONSULTAR_SUSCRIPCION = "CONSULTAR_SUSCRIPCION";
    ...
    @Override
    public RepuestaSPSuscCampaniaBeneficiosEntity consultaCampaniaBeneficiosDAO(SuscCampaniaBeneficiosInDTO inDTO)
            throws DAOException {
        ...
        SqlParameterSource in = new MapSqlParameterSource().addValue(P_NUP, nup).addValue(P_ID_CAMPANIA, idcampania);
        try {
            Map<String, Object> respuesta = consultar(SCHEMA_NAME, PACKAGE_NAME, CONSULTAR_SUSCRIPCION, in,
                    parametros.toArray(new SqlParameter[parametros.size()]));
            BigDecimal codRetorno = new BigDecimal((String) respuesta.get(P_SUSCRIPTO));
            if (codRetorno.intValue() >= 0) {
            ...
            LOGGER.error(ERROR_EN_BASE, SCHEMA_NAME, PACKAGE_NAME, CONSULTAR_SUSCRIPCION, dae);
            throw new DAOException(dae);
        } catch (SQLException sqle) {
            LOGGER.error(MENSAJE_ERROR_EN_STORE, SCHEMA_NAME, PACKAGE_NAME, CONSULTAR_SUSCRIPCION, sqle);
            throw new DAOException(sqle);
        } catch (UncategorizedSQLException ex) {
            LOGGER.error(ERROR_EN_BASE, SCHEMA_NAME, PACKAGE_NAME, CONSULTAR_SUSCRIPCION, ex);
            throw new DAOException(ex);
        } catch (InvalidDataAccessApiUsageException invalidData) {
            LOGGER.error(MENSAJE_ERROR_EN_STORE, SCHEMA_NAME, PACKAGE_NAME, CONSULTAR_SUSCRIPCION, invalidData);
            throw new DAOException(invalidData);
        } catch (IllegalStateException iex) {
            LOGGER.error(MENSAJE_ERROR_EN_STORE, SCHEMA_NAME, PACKAGE_NAME, CONSULTAR_SUSCRIPCION, iex);
            throw new DAOException(iex);
        } catch (QueryTimeoutException qte) {
            LOGGER.error(ERROR_TIMEOUT, SCHEMA_NAME, PACKAGE_NAME, CONSULTAR_SUSCRIPCION, qte);
