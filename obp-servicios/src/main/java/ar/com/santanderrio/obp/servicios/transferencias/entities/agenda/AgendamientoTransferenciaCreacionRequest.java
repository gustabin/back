...
/**
 * The Class AgendamientoTransferenciaCreacionRequest.
 */
public class AgendamientoTransferenciaCreacionRequest extends TransferenciaRequest {

	/**
	 * Instantiates a new agendamiento transferencia creacion request.
	 *
	 ...
	 */
	public AgendamientoTransferenciaCreacionRequest(TransferenciaAgendadaDTO transferenciaAgendadaDTO, Cliente cliente,
	...
	public XMLRequestEntity armarRequestRioRio() {
		...
		XMLRequestEntity request = setearDatosComunesConDatosAdicionalesRequest();
		...
		if (new Integer(cuentaOrigen.getTipoCuenta()).equals(TipoCuenta.CUENTA_UNICA.getCodigo())) {
	...
	public XMLRequestEntity armarRequestOtrosBancos() {
		XMLRequestEntity xmlRequest = setearDatosComunesConDatosAdicionalesRequest();
		...
		parametros.setReferenciaUnivoca(transferenciaAgendadaDTO.getConcepto().getCodigo().concat(transferenciaAgendadaDTO.getDescFromConcepto()));
		...
		if (transferenciaAgendadaDTO.getFiid() == null) {
			...
			parametros.setCuit2(StringUtils.replace(transferenciaAgendadaDTO.getCuit2(), STRING_VACIO_LONGITUD_1, STRING_VACIO));
			parametros.setCuit3(StringUtils.replace(transferenciaAgendadaDTO.getCuit3(), STRING_VACIO_LONGITUD_1, STRING_VACIO));
			...
			parametros.setFiid(StringUtils.leftPad(StringUtils.defaultString(transferenciaAgendadaDTO.getFiid()), 4, STRING_VACIO));
			parametros.setUser(StringUtils.leftPad(StringUtils.defaultString(transferenciaAgendadaDTO.getUser()), 7, STRING_VACIO));
			parametros.setBancoReceptor(transferenciaAgendadaDTO.getBancoReceptor());
			parametros.setBancoDestino(transferenciaAgendadaDTO.getBancoDestino());
		}

		xmlRequest.getDatosAdicionales().getDatosMensAvisos().setTitularCredito(STRING_VACIO);
		datosentrada.setParametros(parametros);
