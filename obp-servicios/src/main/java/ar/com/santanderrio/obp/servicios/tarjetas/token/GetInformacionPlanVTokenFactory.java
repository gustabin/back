 ...
 */
@Component("getInformacionPlanVTokenFactory")
public class GetInformacionPlanVTokenFactory implements TokenFactory {
	...
	public Token crearToken(String tokenCode, Entity... entidades) {
		...
		GetInformacionPlanVToken token = new GetInformacionPlanVToken();
		token.setJks(keystore);
		token.setRequiereCertificado(Boolean.TRUE);
		token.setCodigoBanco(TarjetaUtils.obtenerCodigoBanco(cuenta));
		token.setFuncionario(getFuncionario(cuenta, tokenCode));
		token.setPassword(getPassword(cuenta, tokenCode));
		token.setNumeroTarjeta(StringUtils.right(cuenta.getNroTarjetaCredito(), DIGITOS_TARJETA));
		return token;
	}
	 ...
	 * 
	 * Ej: GETINFORMACIONPLANV.PASSWORD=
	 * GETINFORMACIONPLANV.PASSWORD.VISA=BANCO072
	 * GETINFORMACIONPLANV.PASSWORD.AMEX=BANCO472
	 * 
	 * Si el ambiente es produccion GETINFORMACIONPLANV.PASSWORD debe tener
	 * valor en los demas ambientes se tomara segun el tipo de tarjeta una de
	 * las otras dos props.
	 * 
	 * Si produccion cambia y GETINFORMACIONPLANV.PASSWORD puede tener valor
