...
/**
 * The Class ComprobanteOrdenCompra.
 */
public class ComprobanteOrdenCompra extends DetalleComprobanteView {
	...
	protected final String suscripcionJasper = "orden-compra.jasper";
	...
	protected void ponerParametros(HashMap<String, Object> parametros) {
		try {
			...
			if(this.getPrecioReferencia() == null){
				parametros.put(precioReferenciaKey, "Sin cotización");
			}else{
				parametros.put(precioReferenciaKey, getPrecioReferencia());
			}
			if(this.getFechaReferencia() == null || this.getHoraReferencia() == null){
				parametros.put(fechaReferenciaKey, null);
			}else{
				parametros.put(fechaReferenciaKey, getFechaReferencia() + " - " + getHoraReferencia());
			}
			...
			this.setTituloComprobante("Comprobante de orden de compra de Titulos Valores");
