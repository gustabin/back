...
public class OperacionRepudiarAvalEcheq extends OperacionAval {
	private static final Logger LOGGER = LoggerFactory.getLogger(OperacionRepudiarAvalEcheq.class);

    public OperacionRepudiarAvalEcheq(EcheqApiConnector apiConnector) {
        super(apiConnector);
    }

	@Override
	public OperacionEcheqEnum getOperacion() {
		return OperacionEcheqEnum.REPUDIAR_AVAL;
	}
	...
	public void execute() throws DAOException, ParseException {
		...
		final GuarantyOrderCreationResponse response = echeqApiConnector.postGuarantyRepudiationOrder(request);
	...
	public String getMensajeOk(ConfirmarOperacionInDTO confirmarOperacionInDTO, MensajeBO mensajeBO) {
		return mensajeBO.obtenerMensajePorCodigo(CodigoMensajeConstantes.ECHEQ_FEEDBACK_OK_REPUDIAR_AVAL,
				cheque.getChequeNumero()).getMensaje();
	}

	@Override
	public String getMensajeError(String errorCodeItax) {
		return CodigoMensajeConstantes.ECHEQ_FEEDBACK_ERROR_REPUDIAR_AVAL;
	}

	@Override
	public String getCodigoEstadistica() {
        return EstadisticasConstants.ECHEQ_REPUDIAR_AVAL;
    }
